<script lang="ts">
  ...
  import type { Field as FieldType, Game } from "$lib/types/client-types";
  ...
  interface Props {
    game: Game;
    field: FieldType;
    positions: Promise<PositionData>;
  }

  const { game, field, positions }: Props = $props();

  let isLoadingPositionData = $state(true);
  $effect(() => {
    positions.then(() => {
      isLoadingPositionData = false;
    });
  });
...
<div class="flex flex-col gap-6 self-center w-full max-w-200">
  ...
  <PlaybackControl isLoading={isLoadingPositionData} />
</div>
